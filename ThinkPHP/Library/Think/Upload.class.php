...
class Upload {
    ...
    public function upload($files='') {
        ...
        foreach ($files as $key => $file) {
            ...
            if(in_array($ext, array('jpg','gif','jpeg','bmp','png','swf','doc','docx','pdf','txt'))) {
                // $imginfo = getimagesize($file['tmp_name']);//获取图像大小及相关信息
				$imginfo = $file['tmp_name'];
            ...
            if ($this->uploader->save($file,$this->replace)) {
                ...
                $file['urlpath'] = __ROOT__ . '/' . 'Public/Uploads/' . $file['savepath'] . $file['savename'];
                $info[$key] = $file;
            } else {
                $this->error = $this->uploader->getError();
            }
